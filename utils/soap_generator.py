...
def generate_soap_note():
    """Generate SOAP note using smart templates."""
    ...
    try:
        ...
        soap_note = generator.generate_soap_note(student_id, session_date, session_data)
        
        return jsonify(soap_note)
        
    except Exception as e:
        current_app.logger.error(f'Error generating SOAP note: {str(e)}')
        return jsonify({'error': 'Failed to generate SOAP note'}), 500

@soap_bp.route('/api/soap/templates/<section>')
@require_auth  
def get_soap_templates(section):
    """Get available templates for a SOAP section."""
    
    try:
        generator = SOAPGenerator()
        category = request.args.get('category')
        templates = generator.get_available_templates(section, category)
